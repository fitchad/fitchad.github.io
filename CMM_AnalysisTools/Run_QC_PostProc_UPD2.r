...
################ added ACF 10/7 ##########################
#this section parses the study ID from the name and starts the for loop
#used to generate results by study.
...
for(i in 1:num_exp_ids){

if(i==1){
#skipping, as this is the All_Exp condition
}else{

exp_ix=grep(paste("^",exp_ids_study_uniq[i], sep=""),full_tab[,"SampleID"])#add '^' to prevent matching in middle of line
...
pdf(paste(post_qc_res_dir, "/", LOG_FILE_NAME, ".", exp_ids_study_uniq[i], ".summary.pdf", sep=""), height=11, width=8.5);
...
plot_text(c(
	...
	paste("Experiment Study ID: ",exp_ids_study_uniq[i], sep=""),
...
export_redos=function(fname_root, counts_wnames, cutoffs){
	...
	for(cutix in cutoffs){
		...
		if(is.infinite(cutix)){
			fname=paste(fname_root, ".",exp_ids_study_uniq[i], ".paird_cnts.ALL", ".tsv", sep="");
		}else{
			fname=paste(fname_root, ".",exp_ids_study_uniq[i], ".paird_cnts.lt_", sprintf(spf_str, cutix), ".tsv", sep="");
		}
