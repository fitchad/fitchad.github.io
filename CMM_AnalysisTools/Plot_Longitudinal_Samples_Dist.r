...
usage = paste(
	...
	"	[-T <tag name>]\n",
	"\n",
	"	[-M <Mark events, event column-char list>]\n",
        "\n",
	"	Diversity types include:\n",
	"		shannon, tail, simpson, invsimpson\n",
	"\n",
	"\n",
	"To designate events, use the -M flag and supply a comma separated string of even categories and pch designations\n",
	"For boolean events, supply a category and and a pch character designation, such as 'UTI=8'",
	"\n",
	"For non-boolean events, supply the category and NA, such as 'age=NA'",
	"\n",
	"Multiple events can be indicated on each plot with a string such as 'UTI=8,age=NA'",
	"\n",
	"\n"
...
if(length(opt$mark_event)){
	...
	if(num_event_types==0){
		cat("Improper list supplied, events won't be marked");
	}else{
		EventList=unlist(strsplit(Events, split=","));
...
plot_event=function(x, y, events, markers, col="black"){
	...
	#one issue is that each individual set is being fed to this script as the "events" which would
...
################## acf changes or edits  - added on 4/19/23 from old scipt #########

                ###### had to add a name vector, as R will automatically save a single dim list of integers as an unnamed integer vector.
		#### added if statemnt on 6/6/23, otherwise creates an error if no events specified.
                if(num_event_types>0){
        	        events_mat = as.matrix(factor_info[subset_samples, c(EventCatList)])
	                rownames(events_mat)=c(subset_samples)
                ...
                ###### Creating legend for plot, using dummy values if no legend.
                ...
                if(num_event_types>0){
                        plotD=TRUE;
                        legendN=c(EventCatList)
                        pchN=c(EventCharList)
                }
...
######################################################################
                ...
                plot(offset_info[,"Offsets"], subset_diversity,
                       legend=legend("topright", legend=legendN, plot=plotD, pch=as.numeric(pchN)),
...
#### commented out 4/19/23
#		if(num_event_types>0){
#			events_mat = as.data.frame(factor_info[subset_samples, c(EventCatList)])
#			rownames(events_mat)=subset_samples
#			print("******************************************************************");
#			print(events_mat)
#		}
...
#############added 4/18/23 from old ver ##had to add a name vector, as R will automatically save a single dim list 
#############of integers as an unnamed integer vector.

			if(num_event_types>0){
                        events_mat = as.matrix(factor_info[subset_samples, c(EventCatList)])
                        names(events_mat)=c(subset_samples)
			}
