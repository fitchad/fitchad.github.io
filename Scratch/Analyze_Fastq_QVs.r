...
is_reasonable_qv=function(qvint){
	if(all(qvint>=0 & qvint<=93)){ #edited to 70 acf 6/8/23
		return(T);
	}else{
		return(F);
	}
...
plot_qv_by_pos=function(pos_med_ci, max_len){
	...
	max_disp_qv=93;		###################set from 65 to 93 acf 6/8/23
...
plot_frequency_of_qv=function(all_qv){
	qv_max=93;		#############################changed from 50 to 93
...
plot_qv=function(qv_ci, sample_names, label.cex=1, sorted=F){
	...
	if(sorted){
		...
		qv_ci=t(as.matrix(qv_ci[samp_ix,])); ##edited by ACF to force to matrix and then transpose to [1,3], else fails in plotting below
		sample_names=sample_names[samp_ix];
	}
...
plot_len=function(len_ci, sample_names, label.cex=1, sorted=F){
	...
	if(sorted){
		...
		len_ci=t(as.matrix(len_ci[samp_ix,]));  ##edited by ACF to force to matrix and then transpose to [1,3], else fails in plotting below
		sample_names=sample_names[samp_ix];
	}
