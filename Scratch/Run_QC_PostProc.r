...
for(exp_id in exp_ids_study_uniq){
	...
	###############################################################################
	#Outputting sampleIDs with zero paired reads.

	cat("\n");
	cat("Experiment", exp_id, "samples with 0 paired reads:\n");
	zero_reads=setdiff(uniq_exp_ids,paired_merged_samp_id);
	
	print(zero_reads);
	cat("\n","\n");
	write.table(zero_reads, file=paste(post_qc_res_dir, "/", LOG_FILE_NAME, ".", exp_id, ".paird_cnts.zero_reads.tsv", sep=""), 
