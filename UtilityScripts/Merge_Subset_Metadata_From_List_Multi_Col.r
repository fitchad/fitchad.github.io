...
if(!length(opt$slcolname)){
  ...
  SampleColName = unlist(strsplit(SampleColName, ","))
...
if(!length(opt$mdcolname)){
  ...
  MetadataColName = unlist(strsplit(MetadataColName, ","))  
}
...
# Setting the merge columns to handle multiple columns (vector of column names)
BY.X = SampleColName
if (is.null(SampleColName)) {
  BY.X = colnames(CountsMat)[1]  # Use the first column if not specified
}

BY.Y = MetadataColName
if (is.null(MetadataColName)) {
  BY.Y = colnames(MetaDataFile)[1]  # Use the first column if not specified
}

# If you have more than one column, you'll specify them as vectors
cat("\n")
cat("Merge Columns for Sample List: ", paste(BY.X, collapse = ", "), "\n")
cat("Merge Columns for Metadata: ", paste(BY.Y, collapse = ", "), "\n")
cat("\n")
...
# Merging Summary Table Reads and Metadata File using multiple columns
